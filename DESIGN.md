---
version: beta-3d-professional
name: Apple-design-analysis
description: A photography-first interface that turns marketing into a museum gallery, upgraded with restrained spatial depth. Edge-to-edge product tiles, SF Pro typography, the single Action Blue (#0066cc), and quiet UI chrome remain unchanged. Professional 3D is introduced only through shallow perspective, material-aware highlights, layered translucency, controlled parallax, and low-amplitude elevation. Effects clarify hierarchy and product tactility without becoming decorative, game-like, glossy, or visually noisy.
...
effects-colors:
  ambient-light: "rgba(255, 255, 255, 0.72)"
  ambient-light-soft: "rgba(255, 255, 255, 0.34)"
  ambient-dark: "rgba(0, 0, 0, 0.18)"
  ambient-dark-soft: "rgba(0, 0, 0, 0.08)"
  edge-highlight-light: "rgba(255, 255, 255, 0.78)"
  edge-highlight-dark: "rgba(255, 255, 255, 0.12)"
  glass-light: "rgba(255, 255, 255, 0.72)"
  glass-dark: "rgba(29, 29, 31, 0.72)"
  reflection-blue: "rgba(0, 102, 204, 0.12)"
  shadow-contact: "rgba(0, 0, 0, 0.16)"
  shadow-ambient: "rgba(0, 0, 0, 0.08)"
  shadow-dark-surface: "rgba(0, 0, 0, 0.32)"
  focus-glow: "rgba(0, 113, 227, 0.22)"
...
depth:
  ...
  raised-xs:
    ...
    boxShadow: "0 1px 2px rgba(0, 0, 0, 0.04), 0 6px 18px rgba(0, 0, 0, 0.05)"
  raised-sm:
    transform: "translate3d(0, -2px, 0)"
    boxShadow: "0 2px 5px rgba(0, 0, 0, 0.06), 0 14px 34px rgba(0, 0, 0, 0.08)"
  floating-md:
    transform: "translate3d(0, -4px, 0)"
    boxShadow: "0 4px 10px rgba(0, 0, 0, 0.08), 0 24px 60px rgba(0, 0, 0, 0.10)"
  product-rest:
    transform: "translate3d(0, 0, 0)"
    filter: "drop-shadow(3px 8px 28px rgba(0, 0, 0, 0.20))"
  product-float:
    transform: "translate3d(0, -6px, 24px) rotateX(1.2deg)"
    filter: "drop-shadow(3px 18px 38px rgba(0, 0, 0, 0.22))"

perspective:
  scene-subtle: 1200px
  scene-card: 900px
  scene-product: 1400px
  max-rotate-x: 2deg
  max-rotate-y: 3deg
  max-parallax: 12px
  mobile-max-parallax: 4px

motion:
  duration-instant: 100ms
  duration-fast: 180ms
  duration-standard: 280ms
  duration-spatial: 420ms
  easing-standard: "cubic-bezier(0.22, 1, 0.36, 1)"
  easing-exit: "cubic-bezier(0.4, 0, 1, 1)"
  easing-press: "cubic-bezier(0.2, 0, 0, 1)"
...
materials:
  glass-light:
    ...
    boxShadow: "inset 0 1px 0 rgba(255, 255, 255, 0.72), 0 8px 28px rgba(0, 0, 0, 0.06)"
  glass-dark:
    backgroundColor: "rgba(29, 29, 31, 0.72)"
    backdropFilter: "saturate(140%) blur(20px)"
    border: "1px solid rgba(255, 255, 255, 0.10)"
    boxShadow: "inset 0 1px 0 rgba(255, 255, 255, 0.10), 0 12px 34px rgba(0, 0, 0, 0.24)"
  pearl-raised:
    backgroundColor: "{colors.surface-pearl}"
    border: "1px solid rgba(255, 255, 255, 0.78)"
    boxShadow: "inset 0 1px 0 rgba(255, 255, 255, 0.92), inset 0 -1px 0 rgba(0, 0, 0, 0.035), 0 8px 22px rgba(0, 0, 0, 0.055)"
  dark-satin:
...
components:
  ...
  spatial-product-stage:
    backgroundColor: transparent
    perspective: "{perspective.scene-product}"
    transformStyle: preserve-3d
    isolation: isolate
    maxParallax: "{perspective.max-parallax}"
  spatial-hero-copy:
    transform: "translate3d(0, 0, 28px)"
    transition: "transform {motion.duration-spatial} {motion.easing-standard}"
  spatial-product-render:
    transform: "translate3d(0, 0, 12px)"
    filter: "drop-shadow(3px 8px 28px rgba(0, 0, 0, 0.20))"
    transition: "transform {motion.duration-spatial} {motion.easing-standard}, filter {motion.duration-spatial} {motion.easing-standard}"
  spatial-product-render-active:
    transform: "translate3d(0, -4px, 24px) scale(1.008)"
    filter: "drop-shadow(3px 16px 36px rgba(0, 0, 0, 0.22))"
  utility-card-raised:
    backgroundColor: "{colors.canvas}"
    textColor: "{colors.ink}"
    rounded: "{rounded.lg}"
    border: "1px solid rgba(0, 0, 0, 0.055)"
    boxShadow: "inset 0 1px 0 rgba(255, 255, 255, 0.90), 0 1px 2px rgba(0, 0, 0, 0.035), 0 12px 28px rgba(0, 0, 0, 0.055)"
    transform: "translate3d(0, 0, 0)"
  utility-card-raised-active:
    transform: "translate3d(0, 1px, 0) scale(0.992)"
    boxShadow: "inset 0 1px 0 rgba(255, 255, 255, 0.82), 0 1px 3px rgba(0, 0, 0, 0.05), 0 8px 18px rgba(0, 0, 0, 0.05)"
  trust-proof-card:
    ...
    boxShadow: "inset 0 1px 0 rgba(255, 255, 255, 0.86), 0 12px 30px rgba(0, 0, 0, 0.065)"
  trust-proof-card-on-dark:
    backgroundColor: "rgba(39, 39, 41, 0.74)"
    textColor: "{colors.on-dark}"
    rounded: "{rounded.lg}"
    border: "1px solid rgba(255, 255, 255, 0.10)"
    backdropFilter: "saturate(130%) blur(18px)"
    boxShadow: "inset 0 1px 0 rgba(255, 255, 255, 0.08), 0 18px 44px rgba(0, 0, 0, 0.24)"
  ...
  focus-ring-spatial:
    ...
    outlineOffset: 3px
    boxShadow: "0 0 0 5px rgba(0, 113, 227, 0.16)"
  section-ambient-light:
    backgroundColor: "{colors.canvas}"
    overlay: "radial highlight limited to product stage; opacity <= 0.16"
  section-ambient-dark:
    backgroundColor: "{colors.surface-tile-1}"
    overlay: "localized specular reflection behind product; opacity <= 0.10"
  footer:
    backgroundColor: "{colors.canvas-parchment}"
    textColor: "{colors.ink-muted-80}"
...
Density is unusually low even by contemporary SaaS standards. Each tile occupies roughly one viewport, and there is no decorative chrome — no decorative frames, no shadows on headlines, and no broad ornamental gradients. Elevation remains product-led, but the upgraded system allows shallow card elevation, inset edge highlights, and localized material reflections when they clarify interaction or trust. Product imagery retains the strongest shadow and spatial separation. The result is a catalog that feels more like a museum gallery: the wall disappears and the artifact takes over.
...
- Whisper-soft spatial depth: product imagery receives the strongest elevation; cards and proof surfaces receive only low-amplitude ambient shadows and inset highlights.
...
**Shadow philosophy.** The baseline system gives photographic product imagery the dominant shadow. The professional 3D extension permits only shallow ambient card shadows and inset material highlights. Buttons and text remain visually flat; product imagery remains the strongest depth cue. Elevation in the UI comes from (a) surface-color change (light tile ↔ dark tile) and (b) backdrop-blur on sticky bars. The single shadow is about giving the product weight, not about UI hierarchy.
...
- Don't add heavy shadows to cards, buttons, or text. Cards may use the documented low-amplitude ambient elevation; product imagery must remain the strongest depth cue.
- Don't use broad gradients as decorative backgrounds. Localized specular highlights are allowed only inside a defined product stage or material surface and must stay below the documented opacity limits.
...
## Professional 3D & Effects Extension

### Design Intent
The 3D upgrade must make the interface feel engineered, credible, and materially precise. Depth is not a visual theme. Depth communicates three things only: hierarchy, interactivity, and proof. Existing strengths remain intact: photography-first composition, light/dark tile rhythm, single Action Blue, confident typography, generous whitespace, and restrained UI chrome.

The target is **quiet spatial realism**, not futuristic spectacle. A user should notice that the interface feels premium before noticing a specific effect. No neon bloom, holographic rainbow, gaming HUD, exaggerated glassmorphism, spinning 3D cards, or deep perspective tunnels are permitted.

### Depth Hierarchy
1. **Canvas layer, z = 0.** Full-bleed light, parchment, or dark tile. No ornamental shadow.
2. **Ambient layer, z = 4–8px.** Localized light field behind a product or trust card. Never covers an entire page.
3. **Content layer, z = 12–20px.** Headline, CTA row, status chips, and utility controls.
4. **Product layer, z = 20–32px.** Product render, receipt artifact, dashboard preview, or verified proof object.
5. **Transient layer, z = 40px.** Sticky action bar, modal, popover, or transaction confirmation.

Do not create more than three visibly separated depth planes inside one section. Too many planes reduce trust and make the layout feel promotional rather than professional.

### Surface Materials
- **White ceramic:** Default white card with a single inset top highlight and a shallow ambient shadow. Use for utility cards, dashboards, pricing, and workflow summaries.
- **Parchment glass:** Frosted parchment at 72–80% opacity. Use for sticky bars, sub-navigation, filters, and compact floating controls.
- **Dark satin:** Near-black surface with low-contrast edge light. Use for dark proof cards, blockchain status, and product tiles. Avoid mirror-like reflections.
- **Transparent product stage:** No visible card around the product. Perspective, contact shadow, and a localized light field create the stage.

### Controlled Specular Effects
Specular effects are allowed only as local highlights:
- A 1px inset highlight on the top edge of a raised light card.
- A low-opacity curved reflection inside dark satin or glass.
- A narrow Action Blue reflection near a verified control or transaction proof.
- A soft radial light behind a product render with opacity no higher than 0.16 on light surfaces or 0.10 on dark surfaces.

Specular effects must never reduce text contrast, cross multiple cards, or become a permanent animated shimmer.

### Perspective & Parallax
- Apply `perspective: 1200–1400px` to a dedicated scene wrapper, never to the page root.
- Maximum pointer tilt is `rotateX(2deg)` and `rotateY(3deg)`.
- Maximum desktop parallax displacement is 12px. Mobile displacement is at most 4px.
- Headline and CTA movement must be lower than product movement. The text plane should feel stable.
- Perspective resets to neutral when pointer leaves, focus moves, scrolling begins, or reduced motion is enabled.
- Do not use pointer tilt for forms, legal text, data tables, transaction details, or destructive actions.

### Motion Grammar
- Press: 100–180ms using `{motion.easing-press}`.
- Surface settle: 280ms using `{motion.easing-standard}`.
- Product float or scene return: 420ms using `{motion.easing-standard}`.
- Use opacity and transform only where possible. Avoid animating blur, large shadows, width, height, or layout positions continuously.
- Scroll reveal uses 8–16px vertical travel and a single fade. No repeated reveal after the first exposure.
- One section may have one primary motion event. Supporting elements remain quieter.

### Professional Trust Surfaces
Use `{component.trust-proof-card}` for transaction proofs, verified receipts, escrow summaries, compliance notes, and architecture evidence. Trust cards must prioritize readability over spectacle.

Required characteristics:
- Stable front-facing orientation.
- Clear hash, state, timestamp, network, or verification label.
- Optional shallow depth, never pointer tilt.
- Blue reflected edge only for a verified or actionable state.
- Monospace data may be used for hashes, but body typography remains SF Pro Text or Inter.

### 3D Product Stage Pattern
A product stage consists of:
1. Section canvas.
2. Localized ambient light field.
3. Stable copy plane using `{component.spatial-hero-copy}`.
4. CTA plane.
5. Product or artifact plane using `{component.spatial-product-render}`.
6. Contact shadow aligned with the apparent surface.

The product plane may float on entry or pointer movement. The copy plane must not rotate. CTA controls may translate no more than 1px on active press.

### Accessibility & Reduced Motion
- Respect `prefers-reduced-motion: reduce`. Set transition duration to `{motion.reduced-motion-duration}`, disable tilt, parallax, floating loops, and scroll-linked transforms.
- Never encode status using motion or depth alone. Pair with text, icon, or color.
- Focus outlines remain stronger than decorative glow.
- Do not apply blur behind text if contrast falls below accessibility requirements.
- Keyboard focus must not trigger pointer tilt.
- Preserve touch targets at 44 × 44px minimum.

### Performance Budget
- Above-the-fold 3D should use DOM/CSS transforms or a pre-rendered transparent image. Do not introduce WebGL by default.
- Use `will-change` only during an active interaction, then remove it.
- Limit backdrop blur to sticky navigation, floating controls, or a maximum of two visible trust cards.
- Avoid multiple large blurred layers on mobile.
- Optimize product renders as AVIF/WebP with transparent PNG only when necessary.
- Disable non-essential effects on low-power devices or when frame rate becomes unstable.

### Effect Density Rules
- One dominant spatial effect per viewport.
- Maximum two raised UI surfaces visible at the same hierarchy level.
- Maximum one glass layer over photography.
- No simultaneous tilt, shimmer, glow, and parallax on the same object. Choose one primary and one supporting effect.
- Dark sections use fewer reflections than light sections because contrast already creates depth.

### Implementation Guardrails
- 3D effects must live in enhancement wrappers. Core layout and reading order must work without transforms.
- Components remain usable if JavaScript is disabled; only pointer-reactive enhancements may disappear.
- Do not place critical buttons on moving planes.
- Do not animate blockchain confirmation, payment success, legal agreement, or error messages in a way that can imply a false state.
- Verified effects trigger only after the application receives a trusted state from the backend.

## Updated Do's and Don'ts for 3D

### Do
- Preserve the existing tile architecture, typography, spacing, radii, and the single Action Blue.
- Use perspective only inside a product or artifact stage.
- Use shallow shadows with multiple low-opacity layers rather than one heavy shadow.
- Use inset highlights to suggest material without introducing a second accent color.
- Make transaction proof and receipt cards stable, readable, and front-facing.
- Keep product or artifact imagery as the strongest depth plane.
- Disable spatial motion under reduced-motion preferences.

### Don't
- Don't turn the interface into a game dashboard, crypto casino, or neon sci-fi scene.
- Don't use large glowing borders, rainbow gradients, animated noise, particle fields, or floating decorative objects.
- Don't rotate forms, tables, proof details, terms, or legal content.
- Don't use deep card stacks or exaggerated perspective.
- Don't let effects compete with the product image, CTA, status, or proof.
- Don't animate success until the backend has confirmed the trusted state.
- Don't apply 3D uniformly; flat space is required for contrast and authority.
...
6. Product photography keeps the strongest shadow. UI surfaces may use only the documented `raised-xs`, `raised-sm`, glass, or inset material treatments.
...
## 3D Upgrade Validation Checklist
- [ ] Existing light/dark tile rhythm remains unchanged.
- [ ] Action Blue remains the only interactive accent.
- [ ] Product imagery remains the strongest depth cue.
- [ ] No text, buttons, or legal content is rotated.
- [ ] Card shadows use only documented low-amplitude tokens.
- [ ] Local highlights remain below opacity limits.
- [ ] Reduced-motion mode disables tilt and parallax.
- [ ] Interaction still works without JavaScript enhancement.
- [ ] Mobile uses reduced parallax and fewer blur layers.
- [ ] Focus indicators remain visible and independent from decorative glow.
- [ ] Verified-state visual effects occur only after trusted backend confirmation.
- [ ] No new accent color, neon glow, gaming HUD, or decorative full-page gradient was introduced.
