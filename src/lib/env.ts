...
export const serverEnvSchema = z.object({
  ...
  AI_API_KEY: z.preprocess(emptyStringToUndefined, z.string().optional()),
  AI_MODEL: z.preprocess(emptyStringToUndefined, z.string().optional()),
  METAPLEX_MINT_AUTHORITY_SECRET: z.preprocess(emptyStringToUndefined, z.string().optional()),
  RECEIPT_METADATA_BASE_URL: z.preprocess(emptyStringToUndefined, z.string().url().optional()),
...
export function validateClientEnv(env: Record<string, unknown> = process.env): ClientEnv {
  ...
  if (!result.success) {
    throw new Error(`Client environment validation failed: ${result.error.issues.map((issue) => issue.path.join(".") || "unknown").join(", ")}`);
  }
  return result.data;
}
...
export function validateServerEnv(env: Record<string, unknown> = process.env): ServerEnv {
  ...
  if (!result.success) {
    throw new Error(`Server environment validation failed: ${result.error.issues.map((issue) => issue.path.join(".") || "unknown").join(", ")}`);
  }
  return result.data;
}
